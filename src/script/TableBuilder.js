...
export default class TableBuilder {
	...
	buildTable() {
		...
		this.tableIdString = "";
		if (this.idParser.getPatternOccurence() > 0) {
			this.idParser.parse();

			// set required data according to pattern occurence in element id
	...
	buildTableBody(tableMainHeader, tableDataInJson, properties){
		...
		for (let k = 0; k < tableDataInJson.length; k++) {
			...
			if (typeof(tableDataInJson[k].data) !== 'undefined') {
				...
				TableMainBody += "<tr id=\"" + this.tableIdString + "_row_" + parseInt(k, 10) + "\" >";
				...
				for (let l = 0; l < properties.length; l++) {
					if (Object.keys(tableDataInJson[k].data).indexOf(properties[l]) === -1) {
						TableMainBody += "<td></td>";
					} else {
						TableMainBody += "<td>" + tableDataInJson[k].data[properties[l]] + "</td>";
					}
				}
	...
	buildTableContent(tableDataInJson) {
		...
		if (Array.isArray(tableDataInJson)){
			...
			tableHeader = "<table class=\"table table-striped table-bordered cell-border\" id=\"" + this.tableIdString + "\">" + this.buildTableHeader("", tableDataInJson, properties);
			return tableHeader + this.buildTableBody(tableHeader, tableDataInJson, properties) + "</table>";
		}
	}
